...
export async function pushToAlgolia(records, toRemoveIds) {
  ...
  if (!index) {
    logger.log(
      "INFO", "ALGOLIA_APP_ID / ALGOLIA_ADMIN_API_KEY not set - running in DRY RUN mode. No requests will be sent to Algolia."
    );
    logger.log(
  ...
  try {
    for (let i = 0; i < records.length; i += BATCH_SIZE) {
      ...
      logger.log(
        "INFO", `Saved batch of ${batch.length} records (${Math.min(i + BATCH_SIZE, records.length)}/${records.length}).`
      );
    }
    ...
    if (toRemoveIds.length > 0) {
      ...
      logger.log(
        "INFO", `Deleted ${toRemoveIds.length} stale records from the index.`
      );
    }
